...
{
    public const THUMB_TYPES = ['small', 'small2x', 'medium', 'medium2x'];
    ...
    public function handle()
    {
        ...
        $multiplier = 1;
        $basicType = $type;
        if (($split = \mb_strpos($basicType, '2')) !== false) {
            $basicType = \mb_substr($basicType, 0, $split);
            $multiplier = 2;
        }
        ...
        foreach ($photos as $photo) {
            if ($this->photoFunctions->resizePhoto($photo, $type, $maxWidth, $maxHeight)
                ...
                $this->line(
                    '   Could not create ' . $type . ' for ' . $photo->title . ' (' . $photo->width . 'x' . $photo->height . ').'
                );
            }
            $bar->advance();
        }
