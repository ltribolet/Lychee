...
class SharingController extends Controller
{
     ...
     * @return array<mixed>
     */
    public function listSharing(): array
    {
        $UserId = $this->sessionFunctions->id();
        if ($UserId === 0) {
            $shared = DB::table('user_album')
                ->select('user_album.id', 'user_id', 'album_id', 'username', 'title', 'parent_id')
            ...
            $albums = Album::select(['id', 'title', 'parent_id'])
                ->where('owner_id', '=', $UserId)->orderBy('title', 'ASC')->get()->each(function ($album): void {
                    $album->title = Album::getFullPath($album);
                });
