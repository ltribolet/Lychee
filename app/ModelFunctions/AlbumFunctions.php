...
class AlbumFunctions
{
     ...
     */
    public function is_smart_album(int $albumID): bool
    {
     ...
     */
    public function set_thumbs(array &$return, BaseCollection $thumbs): void
    {
        $return['thumbs'] = [];
        $return['types'] = [];
        $return['thumbs2x'] = [];

        $thumbs[0]->each(function (Thumb $thumb, $key) use (&$return): void {
            $thumb->insertToArrays($return['thumbs'], $return['types'], $return['thumbs2x']);
        });
    }

    public function set_thumbs_children(BaseCollection $return, BaseCollection $thumbs): void
    {
        $thumbs->each(function (BaseCollection $subthumb, $key) use (&$return): void {
            $mod = $return[$key];
     ...
     * @return array<mixed>
     */
    public function photosLocationData(Builder $photos_sql, bool $full_photo): array
    {
        $return_photos = [];
        $photo_counter = 0;
        $photos = $photos_sql->select(
            'album_id',
            'id',
            'latitude',
            'longitude',
     ...
     * @return array<mixed>
     */
    public function photos(Builder $photos_sql, bool $full_photo, string $license = 'none'): array
    {
        $previousPhotoID = '';
        $return_photos = [];
        $photo_counter = 0;
        ...
        if ($sortingCol === 'title' || $sortingCol === 'description') {
            ...
            \array_multisort(
                $values,
                $sortingOrder === 'ASC' ? SORT_ASC : SORT_DESC,
     ...
     *
     * @param $album : speed optimization to avoid an extra query,
     ...
     *
     * @param array<int>|\Illuminate\Support\Collection $albumIDs
     */
    public function setContentsOwner($albumIDs, int $ownerId): bool
    {
     ...
     */
    public function get_sub_albums(Album $album, bool $includePassProtected = false): BaseCollection
    {
