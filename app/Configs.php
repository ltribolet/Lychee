...
class Configs extends Model
{
    ...
    public static function get_value(string $key, $default = null)
    {
        ...
        if (!isset(self::$cache[$key])) {
             ...
             */
            try {
                Logs::notice(__METHOD__, (string) __LINE__, $key . ' does not exist in config (local) !');
            } catch (\Throwable $e) {
    ...
    public static function set(string $key, $value): bool
    {
        ...
        if ($config === null) {
            Logs::warning(__METHOD__, (string) __LINE__, 'key ' . $key . ' not found!');
        ...
        if ($message !== '') {
            Logs::error(__METHOD__, (string) __LINE__, $message);
    ...
    {
        if (\extension_loaded('imagick') && self::get_value('imagick', '1') === '1') {
            return true;
        }
        try {
            Logs::notice(__METHOD__, (string) __LINE__, 'hasImagick : false');
        } catch (\Throwable $e) {
            //do nothing
        }
    ...
    public static function hasExiftool(): bool
    {
        ...
        $has_exiftool = (int) self::get_value('has_exiftool');
        ...
        if ($has_exiftool === 2) {
            try {
                ...
                self::set('has_exiftool', 0);
                $has_exiftool = false;
                Logs::warning(__METHOD__, (string) __LINE__, 'exec is disabled, has_exiftool has been set to 0.');
            }
    ...
    public static function hasFFmpeg(): bool
    {
        ...
        $has_ffmpeg = (int) self::get_value('has_ffmpeg');
        ...
        if ($has_ffmpeg === 2) {
            try {
                ...
                self::set('has_ffmpeg', 0);
                $has_ffmpeg = false;
                Logs::warning(__METHOD__, (string) __LINE__, 'exec is disabled, set_ffmpeg has been set to 0.');
            }
        } elseif ($has_ffmpeg === 1) {
            $has_ffmpeg = true;
        } else {
