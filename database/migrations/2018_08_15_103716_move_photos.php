    ...
    public function up(): void
    {
        ...
        if (count(Photo::all()) === 0) {
            ...
            if (Schema::hasTable(env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_photos')) {
                $results = DB::table(env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_photos')->select('*')->orderBy(
                    'id',
                    'asc'
                )->orderBy('album', 'asc')->get();
                $id = 0;
                foreach ($results as $result) {
                    ...
                    $photo->takestamp = $result->takestamp === 0 || $result->takestamp === null ? null : date(
                        'Y-m-d H:i:s',
                        $result->takestamp
                    );
                    ...
                    if (Storage::exists('medium/' . $photo->url)) {
                        [$width, $height] = getimagesize(Storage::path('medium/' . $photo->url));
                        $photo->medium = $width . 'x' . $height;
                    } else {
                        $photo->medium = '';
                    }
                    if (Storage::exists('small/' . $photo->url)) {
                        [$width, $height] = getimagesize(Storage::path('small/' . $photo->url));
                ...
                Logs::notice(
                    __FUNCTION__,
                    (string) __LINE__,
                    env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_photos does not exist!'
                );
            }
        } else {
            Logs::notice(__FUNCTION__, (string) __LINE__, 'photos is not empty.');
        }
