    ...
    public function up(): void
    {
        if (count(Album::all()) === 0) {
            if (Schema::hasTable(env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_albums')) {
                $results = DB::table(env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_albums')->select('*')->orderBy(
                    'id',
                    'asc'
                )->get();
                $id = 0;
                ...
                Logs::notice(
                    __FUNCTION__,
                    (string) __LINE__,
                    env('DB_OLD_LYCHEE_PREFIX', '') . 'lychee_albums does not exist!'
                );
            }
        } else {
            Logs::notice(__FUNCTION__, (string) __LINE__, 'albums is not empty.');
        }
    }
